...
import type { AuthUser, Application, DashboardSummary, ApplicationEvent } from "@shared/types";
...
export function userRoutes(app: Hono<{Bindings: Env;}>) {
  ...
  app.get('/api/dashboard-summary', async (c) => {
    ...
    const summary = applications.reduce((acc: DashboardSummary, app: Application) => {
  ...
  app.get('/api/applications', async (c) => {
    ...
    const sortedItems = items.sort((a: Application, b: Application) => new Date(b.applicationDate).getTime() - new Date(a.applicationDate).getTime());
  ...
  app.get('/api/application/:id', async (c) => {
    ...
    if (application.events) {
      application.events.sort((a: ApplicationEvent, b: ApplicationEvent) => new Date(b.date).getTime() - new Date(a.date).getTime());
    }
